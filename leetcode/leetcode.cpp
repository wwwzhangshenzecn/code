...
vector<pair<int, int>> findStones(const vector<int>& stones, int pos, int step) {
	//第一步必须只能跳1的距离
	if (pos == 0) {
	...
	for (int i = pos + 1; i < stones.size(); i++) {
		...
		if (diff >= step - 1 && diff <= step + 1) {
			results.push_back({ i, stones[i] - stones[pos] });
		}
		if (diff >= step + 1) break;
	}
	return results;
}

//403. 递归,极端情况下复杂度会很差很差
bool canCross1(vector<int>& stones, int pos = 0, int step = 1) {
	if (pos < 0 || pos >= stones.size())
		return false;
	if (pos == stones.size() - 1)
		return true;
	if (pos == 0)
		for (int i = 1; i < stones.size(); i++) {
			if (stones[i] - stones[i - 1] > step + i - 1)
				return false;
		}

	vector<pair<int, int>> nextStones = findStones(stones, pos, step);
	for (vector<pair<int, int>>::iterator iter = nextStones.begin();
	iter != nextStones.end(); iter++) {
		if (canCross1(stones, iter->first, iter->second))
			return true;
	}
	return false;
}
...
bool canCross(vector<int>& stones) {
	...
	while (!stack.empty()) {
		...
		if (pos.first == stones.size() - 1)
			return true;
		for (int i = pos.first + 1; i < stones.size(); i++) {
			int diff = stones[i] - stones[pos.first];
			if (diff >= pos.second - 1 && diff <= pos.second + 1) {
				stack.push_back({ i, stones[i] - stones[pos.first] });
			}
			if (diff >= pos.second + 1) break;
		}
	}
	return false;
}
//404
void MidOrderTreeCaluLeftValue(TreeNode* root, bool flag, int& result) {
	if (root != nullptr) {
		if (root->left == nullptr && root->right == nullptr && flag) 
			result += root->val;
		MidOrderTreeCaluLeftValue(root->left, true, result);
		MidOrderTreeCaluLeftValue(root->right, false, result);
	}
}
